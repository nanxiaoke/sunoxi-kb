...
## 2026-06-09 - WebUI 图谱模块后续策略

### 决策
图谱不继续作为本轮 WebUI 拆分的附带任务处理，后续单独开阶段做图谱模块或重做方案。

### 原因
- 当前图谱不是核心路径，继续在本轮重构里拆会增加验证面。
- 图谱渲染适合单独梳理状态、渲染入口、resize/theme 交互和可视化测试。
- 现有未跟踪 `scripts/webui/static/js/modules/graph.js` 草稿保持不纳入提交，避免把半成品模块化方案固化。
